    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-200 dark:bg-gray-900 p-4">
      <div className="w-full max-w-md bg-white dark:bg-gray-800 rounded-lg shadow-xl p-8 max-h-[90vh] overflow-y-auto">
        ...
        <h2 className="text-2xl font-bold text-center text-gray-800 dark:text-white mb-2">
          {isTakeAwayMode ? 'Registro Take Away' : `Bienvenido a ${companyInfo.name}`}
        </h2>
        <p className="text-center text-gray-600 dark:text-gray-300 mb-8">
          {isTakeAwayMode 
            ? 'Regístrate en un paso para empezar a realizar tus pedidos.' 
        ...
        <form onSubmit={handleEmailAuth} className="space-y-4 mb-6">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full py-3 px-4 bg-primary-600 text-white rounded-md hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors font-bold"
          >
            {isLoading ? 'Procesando...' : (isRegistering ? '¡Registrarme y Empezar!' : 'Iniciar Sesión')}
          </button>
        </form>
        ...
        {isRegistering && !isTakeAwayMode && (
            <div className="mt-4 text-center">
                <button 
                    onClick={() => setIsTakeAwayMode(true)}
                    className="text-xs text-gray-500 hover:underline"
                >
                    ¿Solo quieres comprar para llevar? Registro rápido aquí
                </button>
            </div>
        )}
